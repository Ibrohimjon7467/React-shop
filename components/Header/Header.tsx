...
const Header = ({ handleSearch, resetSearch, searchValue }: Props) => {
  ...
  const { cart, totalItems, totalAmount, removeProduct, bounce, bouceEnd } = useContext<Init>(CartContext);
    ...
    return (
      ...
      >
        <li className={styles.cartItem} key={product.name}>
          ...
          <button className={styles.productRemove} onClick={() => removeProduct(product.id)} type="button">
            ×
          </button>
        </li>
      </CSSTransition>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.search}>
          <button className={styles.mobileSearch}
            onClick={handleMobileSearch} type="button"
          >
          ...
          >
            <button className={styles.backButton}
              onClick={handleSearchNav} type="button"
            >
              <img src="https://res.cloudinary.com/sivadass/image/upload/v1494756030/icons/back.png" alt="back" />
            </button>
            <input type="search" placeholder="Searching products..."
              className={styles.searchKeyword} value={searchValue}
              onChange={handleSearch}
            />
            <button className={styles.searchButton} type="submit"
              onClick={handleSubmit} aria-label="submit"
            />
          </form>
        </div>
        ...
        <div className={styles.cart}>
          ...
          <button className={styles.cartIcon}
            onClick={handleCart} type="button"
          >
            <img className={bounce ? styles.tada : " "}
